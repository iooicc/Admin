...
//	}
	...
	// 预编译XSS过滤正则表达式
	private static List<Pattern> xssPatterns = CollectionUtil.newArrayList(
	...
	public static String xssFilter(String text) {
		String oriValue = StrUtil.trim(text);
		if (text != null){
			String value = oriValue;
			for (Pattern pattern : xssPatterns) {
				Matcher matcher = pattern.matcher(value);
				if (matcher.find()) {
					value = matcher.replaceAll(StrUtil.EMPTY);
				}
			}
			// 如果开始不是HTML，XML，JOSN格式，则再进行HTML的 "、<、> 转码。
			if (!StrUtil.startWithIgnoreCase(value, "<!--HTML-->") 	// HTML
					&& !StrUtil.startWithIgnoreCase(value, "<?xml ") 	// XML
					&& !StrUtil.containsAny(value, "id=\"FormHtml\"") 		// JFlow
					&& !(StrUtil.startWith(value, "{") && StrUtil.endWith(value, "}")) // JSON Object
					&& !(StrUtil.startWith(value, "[") && StrUtil.endWith(value, "]")) // JSON Array
				){
